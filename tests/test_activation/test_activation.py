import unittest
from core.activations import ReLULayer as ReLU
from core.activations import SoftmaxLayer as Softmax
from core.activations import SigmoidLayer as Sigmoid
from core.activations import TanhLayer as Tanh
...
class TestTanh(unittest.TestCase):
    ...
    def test_tanh_forward(self):
        in_tensors = [Tensor(elements=np.array([-2.0, -1.0, 0.0, 1.0, 2.0]))]
        out_tensors = [Tensor(elements=np.zeros(5))]
        self.tanh.forward(in_tensors, out_tensors)
        expected = np.tanh(in_tensors[0].elements)
        self.assertTrue(
            np.allclose(out_tensors[0].elements, expected),
            "Tanh forward does not match expected output",
        )

    def test_tanh_backward(self):
        in_tensors = [Tensor(elements=np.array([-2.0, -1.0, 0.0, 1.0, 2.0]))]
        out_tensors = [Tensor(elements=np.zeros(5))]
        out_tensors[0].deltas = np.array([1, 1, 1, 1, 1])
        self.tanh.forward(in_tensors, out_tensors)  # populate internal state
        self.tanh.backward(out_tensors, in_tensors)
        expected_grad = (1 - np.square(np.tanh(in_tensors[0].elements))) * out_tensors[0].deltas
        self.assertTrue(
            np.allclose(in_tensors[0].deltas, expected_grad),
            "Tanh backward does not match expected gradient",
        )
